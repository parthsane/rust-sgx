...
impl<'a> EnclaveBuilder<'a> {
    ...
    fn load<T: Load>(mut self, loader: &mut T) -> Result<(Vec<ErasedTcs>, usize, usize), Error> {
    ...
    pub fn build<T: Load>(self, loader: &mut T) -> Result<Command, Error> {
        self.load(loader)
            .map(|(t, a, s)| Ok(Command::internal_new(t, a, s)))?
    }

    pub fn build_library<T: Load>(self, loader: &mut T) -> Result<Library, Error> {
        self.load(loader)
            .map(|(t, a, s)| Ok(Library::internal_new(t, a, s)))?
    }
}
