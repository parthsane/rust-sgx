...
/// `#[dependency]`:
/// ```ignore
/// define_dependencies!($path, $type);
/// ```
/// This will define an array of all types in dependencies marked with
/// `#[dependency]`. The type of the array elements will be `(TypeId, TypeId,
/// $type)`. Each element in the array will describe an edge in the dependency
/// graph. For an item `impl Dependency<T> for U`, the first tuple element will
/// be the [`TypeId`] of the dependency `T`, the second value that of the the
/// dependent `U`. The third value will be `$path::<T, U>`, which should be a
/// value of type `$type`.
///
/// [`TypeId`]: https://doc.rust-lang.org/std/any/struct.TypeId.html
#[proc_macro]
pub fn define_dependencies(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let (path, ty) = match (|stream: ParseStream| {
        let path: TypePath = stream.parse()?;
    ...
    if let Some(map) = map {
        ...
        let elems = map.iter().flat_map(|(k, vs)| {
            vs.iter().map(move |v| {
                ...
                quote!((#typeid::of::<#k>, #typeid::of::<#v>, #path::<#k, #v>))
            })
        });
